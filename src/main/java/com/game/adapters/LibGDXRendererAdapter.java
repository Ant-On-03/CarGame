...
import com.game.rendering.terrain.*;
import com.game.rendering.outline.*;
...
public class LibGDXRendererAdapter implements Renderer {
    ...
    // Visual system
    private TerrainVisualConfig terrainVisualConfig;
    private TerrainShaderBinder shaderBinder;
    ...
    public LibGDXRendererAdapter() {
        ...
        terrainShader = new ShaderProgram(
                Gdx.files.internal("terrain.vert"),
                Gdx.files.internal("terrain.frag")
        );

        if (!terrainShader.isCompiled()) {
            Gdx.app.error("ShaderError", terrainShader.getLog());
        }

        // ---------- VISUAL CONFIGURATION ----------

        TerrainLightingConfig lighting = new TerrainLightingConfig();

        TerrainOutlineConfig outline =
                new TerrainOutlineConfig(new DarkenOutlineStrategy(0.75f));

        terrainVisualConfig =
                new TerrainVisualConfig(lighting, outline);

        shaderBinder =
                new TerrainShaderBinder(terrainVisualConfig);
    }
    ...
    public void setCamera(double worldX, double worldY, double pixelsPerMeter) {
        ...
        camera.position.set(
                (float)(cameraWorldX * pixelsPerMeter),
                (float)(cameraWorldY * pixelsPerMeter),
                0
        );
    ...
    public void beginFrame() {
        ...
        if (terrainRenderer != null) {
            ...
            double elevation =
                    terrainRenderer.getElevationAt(cameraWorldX,cameraWorldY);

            int maxLayers =
                    terrainRenderer.getRenderConfig().maxLayers();

            float parallaxScale =
                    (float) terrainRenderer.getRenderConfig().parallaxScaleFactor();

            float currentLayer =
                    (float)(((elevation + 100.0)/200.0) * maxLayers);

            currentLayer =
                    Math.max(0,Math.min(maxLayers - 1,currentLayer));

            camera.zoom =
                    1.0f/(1.0f + (currentLayer * parallaxScale));
    ...
    public void drawCar(
            ...
            double width,double length,double velocity,
            WheelRenderData[] wheels) {

        recordTireMarks(wheels);

        Matrix4 savedMatrix =
                shapeRenderer.getTransformMatrix().cpy();
    ...
    private void drawTerrainGPU() {
        ...
        double chunkSize =
                terrainRenderer.getRenderConfig().chunkSizeMetres();
        ...
        double viewRadiusX =
                (screenW/2.0)/ppm + chunkSize;

        double viewRadiusY =
                (screenH/2.0)/ppm + chunkSize;

        int minChunkX =
                (int)Math.floor((cameraWorldX - viewRadiusX)/chunkSize);

        int maxChunkX =
                (int)Math.floor((cameraWorldX + viewRadiusX)/chunkSize);

        int minChunkY =
                (int)Math.floor((cameraWorldY - viewRadiusY)/chunkSize);
        ...
        shaderBinder.bindGlobalUniforms(terrainShader);

        float baseTexelSize =
                1.0f / terrainRenderer.getRenderConfig().chunkSizePixels();

        for(int z = 0; z < maxLayers; z++) {

            float layerScale =
                    1.0f + (z * parallaxScale);
            ...
            float layerDepth =
                    ((float)z/(float)maxLayers)*0.45f;

            terrainShader.setUniformf("u_layerDepth",layerDepth);

            float texelSize =
                    baseTexelSize * layerScale;
            ...
            for(int cy=minChunkY; cy<=maxChunkY; cy++) {
                for(int cx=minChunkX; cx<=maxChunkX; cx++) {
                    ...
                    Texture chunkTex =
                            terrainRenderer.getChunkTexture(cx,cy);

                    if(chunkTex != null) {

                        float chunkWorldPxX =
                                (float)(cx * chunkSize * ppm);

                        float chunkWorldPxY =
                                (float)(cy * chunkSize * ppm);
    ...
    private void drawTireMarks() {
        ...
        for(TireMark mark : tireMarks) {
            ...
            if(prev != null) {
                ...
                float alpha =
                        (float)mark.age / TIRE_MARK_FADE_FRAMES;
    ...
    private void drawSingleWheel(WheelRenderData w) {
        ...
        float wheelW =
                (float)Math.max(Math.abs(w.offsetX()) * 0.35,3.0);

        float wheelH =
                (float)Math.max(Math.abs(w.offsetY()) * 0.55,6.0);
